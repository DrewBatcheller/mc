...
export function SyncIdeaModal({ isOpen, onClose, onSuccess, idea }: SyncIdeaModalProps) {
  ...
  if (clientId) {
    batchFilterParts.push(`FIND("${clientId}", CONCATENATE({Record ID (from Client)})) > 0`)
  }
  ...
  const { data: rawClient } = useAirtable('clients', {
    ...
    filterExtra: clientId ? `RECORD_ID() = "${clientId}"` : undefined,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'Synced!', description: `"${idea.testDescription}" has been added to the batch.` })
      onSuccess()
      onClose()
    } catch (err) {
