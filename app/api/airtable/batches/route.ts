...
import type { BatchFields } from "@/lib/v2/types"
...
export async function GET(request: Request) {
  try {
    ...
    // If user is a client, only return batches for their own client
    if (session.role === "client") {
      if (!session.clientRecordId) {
        return NextResponse.json({ error: "Client record ID not found" }, { status: 400 })
      }
      filters.push(`FIND("${session.clientRecordId}", ARRAYJOIN({Record ID (from Client)}))`)
    } else {
      // Management/team can see all or filter by clientId if provided
      if (clientId) {
        filters.push(`FIND("${clientId}", ARRAYJOIN({Record ID (from Client)}))`)
      }
    }
    ...
    const records = await fetchTable<BatchFields>(TABLES.BATCHES, {
      filterByFormula,
