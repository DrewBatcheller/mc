...
/**
 * Strategy Review Design Mockups — hosted form at /forms/strategy-review-mockups?id=recBatchID
 *
 * Strategist reviews design mockups submitted by designers for a batch.
 * Each experiment can be individually approved or rejected with feedback.
 * Rejections create a Note (Experiment Type: "Strategy Feedback") linked to the
 * experiment and set Feedback Status to "Strategy Rejected Design".
 * Approvals set Feedback Status to "Strategy Approved Design".
 *
 * Once ALL experiments are approved, the strategist can click "Send to client
 * for review" which will eventually fire an n8n webhook.
 *
 * Rejection does NOT remove the experiment from the batch — the designer
 * revises the mockup and re-submits for review.
 *
...
import {
  ...
  ExternalLink, AlertTriangle, FileText, Figma, Send,
} from 'lucide-react'
...
const PREVIEW_EXPERIMENTS: MockupExperiment[] = [
  {
    ...
    designBrief: 'Sticky bar at bottom of viewport with product name + price + ATC button.',
    devices: 'Mobile',
  ...
  },
  {
    ...
    placement: 'Product Page',
    placementUrl: 'https://store.acmecorp.com/products/example',
    designBrief: 'Three inline badges with icons: Free Shipping, 30-Day Returns, Secure Checkout.',
    devices: 'All Devices',
    figmaUrl: 'https://www.figma.com/design/def456/example',
    feedbackStatus: 'Strategy Approved Design',
    noteIds: ['prev_n1'],
    noteCount: 1,
  },
  {
...
function StrategyReviewMockupsInner() {
  ...
  const { data: rawExperiments, isLoading: experimentsLoading } = useAirtable<Record<string, unknown>>('experiments', {
    fields: [
      'Test Description', 'Placement', 'Placement URL', 'Design Brief',
      'Devices', 'FIGMA Url', 'Feedback Status', 'Notes',
    ],
  ...
  const noteFilter = useMemo(() => {
    ...
    // Show strategy feedback and client design feedback
  ...
  const notesByExperiment = useMemo<Map<string, { note: string; createdTime: string; type: string; author: string }[]>>(() => {
    ...
    for (const rec of rawNotes) {
      ...
      const author = Array.isArray(teamArr) ? String(teamArr[0] ?? '') : Array.isArray(clientArr) ? String(clientArr[0] ?? '') : ''
  ...
  const decisions = useMemo(() => {
    ...
    for (const exp of experiments) {
      if (exp.feedbackStatus === 'Strategy Approved Design') {
        merged.set(exp.id, 'approved')
      } else if (exp.feedbackStatus === 'Strategy Rejected Design') {
  ...
  const handleApprove = async (experimentId: string) => {
    ...
    try {
      const res = await fetch(`/api/airtable/experiments/${experimentId}`, {
        method: 'PATCH',
        headers: authHeaders,
        body: JSON.stringify({ fields: { 'Feedback Status': 'Strategy Approved Design' } }),
      })
      if (!res.ok) {
      ...
      try {
        ...
        await fetch(
          `https://more-conversions.app.n8n.cloud/webhook/585f1960-4886-4a68-b1d1-e6f1fa6fe0ec?experimentID=${experimentId}&batchID=${batchId}&clientID=${clientId}&action=strategy_approved_design`,
          { method: 'GET' },
        )
      ...
      setError(err instanceof Error ? err.message : 'Failed to approve mockup')
    } finally {
  ...
  const handleReject = async (experimentId: string) => {
    ...
    try {
      // 1. Create rejection Note with "Strategy Feedback" type
      const noteFields: Record<string, unknown> = {
        Note: feedbackText.trim(),
        Experiments: [experimentId],
        'Experiment Type': 'Strategy Feedback',
      }
      if (user) {
      ...
      // 2. PATCH experiment — mark as rejected
      const patchRes = await fetch(`/api/airtable/experiments/${experimentId}`, {
        method: 'PATCH',
        headers: authHeaders,
        body: JSON.stringify({ fields: { 'Feedback Status': 'Strategy Rejected Design', 'Rejected At': new Date().toISOString() } }),
      })
      if (!patchRes.ok) {
      ...
      try {
        const clientId = batch?.clientId ?? ''
        await fetch(
          `https://more-conversions.app.n8n.cloud/webhook/585f1960-4886-4a68-b1d1-e6f1fa6fe0ec?experimentID=${experimentId}&batchID=${batchId}&clientID=${clientId}&action=strategy_rejected_design`,
          { method: 'GET' },
        )
      } catch { /* webhook fire-and-forget */ }
  ...
  const handleSendToClient = async () => {
    ...
    setError(null)
    try {
      // TODO: Fire n8n webhook to notify client of mockups ready for review
      // const clientId = batch?.clientId ?? ''
      // await fetch(`WEBHOOK_URL?batchID=${batchId}&clientID=${clientId}&action=mockups_ready_for_client`, { method: 'GET' })
  ...
  if (sentToClient) {
    return (
      <FormSuccess
        title="Mockups Sent to Client"
        description={
          <>
            All approved design mockups for{' '}
            <span className="font-semibold text-neutral-700">{batch.clientName}</span>{' '}
            have been sent to the client for review. They will be notified to review the designs.
          </>
        }
      />
    )
  ...
  const formContent = (
    <>
      ...
      <FormBody>
        ...
        <StepCard num="01" title="Batch Overview">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-neutral-400">Mockups</p>
                <p className="text-[13px] text-neutral-800">{totalCount} {totalCount === 1 ? 'design' : 'designs'} to review</p>
              </div>
            </div>
            {batch.launchDate && (
              <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-3">
              ...
              {rejectedCount > 0 && (
                <span className="text-[12px] font-medium text-red-600">
                  {rejectedCount} revision{rejectedCount !== 1 ? 's' : ''} requested
                </span>
              )}
              {allReviewed && (
          ...
          <div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden">
            <div
              className={cn(
                ...
                allApproved ? 'bg-emerald-500' : allReviewed ? 'bg-sky-500' : 'bg-sky-500'
              )}
        ...
        {/* Step 2 — Experiment mockup cards */}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1 ml-0">
                        ...
                        {exp.figmaUrl ? (
                          ...
                          <span className="text-[10px] text-amber-600 font-medium">No Figma URL</span>
                        )}
                    ...
                    <div className="px-4 pb-4 space-y-4 border-t border-neutral-100 pt-4">
                      ...
                      {exp.figmaUrl ? (
                        <div className="rounded-lg border border-violet-200 bg-violet-50/50 p-4">
                          <p className="text-[11px] font-semibold uppercase tracking-wide text-violet-600 mb-2">Design Mockup</p>
                          <a
                            href={exp.figmaUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-violet-200 bg-white hover:bg-violet-50 transition-colors text-[13px] text-violet-700 font-medium group"
                          >
                            <Figma className="h-4 w-4 text-violet-500 group-hover:text-violet-600" />
                            Open in Figma
                        ...
                        <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-[13px] text-amber-700 font-medium">
                              No Figma URL has been submitted for this experiment yet.
                            </p>
                          </div>
                        </div>
                      )}
                            ...
                            <div key={i} className="text-[13px] text-neutral-700 whitespace-pre-wrap bg-white rounded-lg border border-neutral-100 px-3 py-2">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <span className={cn(
                                  ...
                                  n.type === 'Strategy Feedback' ? 'bg-violet-50 text-violet-600' : 'bg-orange-50 text-orange-600',
                                )}>
                                  {n.type === 'Strategy Feedback' ? 'Strategy' : 'Client'}
                                </span>
                      ...
                      {!isRejecting && decision && (
                        <div className="flex items-center gap-2 pt-2 border-t border-neutral-100">
                          ...
                          >
                            ...
                            {decision === 'approved' ? 'Approved' : 'Revision Requested'}
                          </button>
                          {decision === 'rejected' && (
                            <button
                              type="button"
                        ...
                        <div className="rounded-lg border border-red-200 bg-red-50/50 p-4 space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-[13px] font-medium text-red-800">
                              What revisions are needed for this design?
                            </p>
                          </div>
                          <FormField label="Feedback for designer" required>
                            <textarea
                              value={feedbackText}
                              onChange={e => setFeedbackText(e.target.value)}
                              placeholder="Describe what needs to be changed in the design mockup…"
                              rows={4}
        ...
        {/* Send to client — only when all approved */}
        {allApproved && !sentToClient && (
          <StepCard num="03" title="Send to Client for Review">
            <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4 space-y-3 mb-4">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                  <Check className="h-5 w-5 text-emerald-600" />
                </div>
                <div>
                  <p className="text-[14px] font-semibold text-emerald-800">All Designs Approved</p>
                  <p className="text-[13px] text-emerald-600 mt-0.5">
                    All {totalCount} design {totalCount === 1 ? 'mockup has' : 'mockups have'} been approved.
                    Send them to <span className="font-medium">{batch.clientName}</span> for final review.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              {isSendingToClient ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
              {isSendingToClient ? 'Sending…' : 'Send mockups to client for review'}
            </button>
          </StepCard>
        )}

        {/* Completion banner — when all reviewed but some rejected */}
        {allReviewed && !allApproved && (
          <div className="bg-amber-50 rounded-2xl border border-amber-200 shadow-[0_1px_4px_rgba(0,0,0,0.05)] px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[14px] font-semibold text-amber-800">Review Complete — Revisions Needed</p>
                <p className="text-[13px] text-amber-600 mt-0.5">
                  {approvedCount} {approvedCount === 1 ? 'design' : 'designs'} approved, {rejectedCount} sent back for revision.
                  The designer will be notified to make changes.
                </p>
              </div>
            </div>
          </div>
        ...
        {confirmAction.type === 'approve' ? (
          <>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-neutral-800">Approve this design?</h3>
          ...
          <>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-neutral-800">Request revision for this design?</h3>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col"
        ...
        <div className="px-6 pt-5 pb-4 border-b border-neutral-100 flex items-center justify-between shrink-0">
          <div className="min-w-0 flex-1">
            <h3 className="text-[15px] font-semibold text-neutral-800">Design Notes</h3>
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1">
          <NotesPanel
            ...
            experimentType="Strategy Feedback"
            placeholder="Add feedback about this design…"
...
export default function StrategyReviewMockupsPage() {
  return (
    <FormPage>
      <StrategyReviewMockupsInner />
    </FormPage>
  )
}
