...
function Toggle({
  ...
  return (
    <label htmlFor={id} className="flex items-center gap-3 cursor-pointer select-none group">
      {/* Hidden real checkbox — label click reliably toggles it */}
      <input
        id={id}
        type="checkbox"
        checked={checked}
  ...
  useEffect(() => {
    ...
    fetch(`/api/airtable/team?${qs.toString()}`, {
      headers: {
        ...
        "x-user-id":   "convert-form",
        "x-user-name": "Convert Form",
      },
    })
      .then(r => r.json())
      .then(data => {
            ...
            <>
              <div className="grid grid-cols-2 gap-4">
                {(Object.entries(ROLE_DEPTS) as [string, string][]).map(([role, dept]) => {
                  // Names only for display in SelectField; IDs are resolved at submit time
