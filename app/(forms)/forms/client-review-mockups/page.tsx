...
/**
 * Client Review Design Mockups — hosted form at /forms/client-review-mockups?id=recBatchID
 *
 * Client reviews design mockups that have already been approved by the strategist.
 * Each experiment can be individually approved or rejected with feedback.
 * Rejections create a Note (Experiment Type: "Client Design Feedback") linked to the
 * experiment and set Feedback Status to "Client Rejected Design".
 * Approvals set Feedback Status to "Client Approved Design".
 *
...
import {
  ...
  ExternalLink, AlertTriangle, FileText, Figma,
} from 'lucide-react'
...
const PREVIEW_EXPERIMENTS: MockupExperiment[] = [
  ...
  },
  {
    ...
    hypothesis: 'If we add trust badges below the price, then CVR will increase because users will feel more confident purchasing.',
    placement: 'Product Page',
    placementUrl: 'https://store.acmecorp.com/products/example',
  ...
  },
  {
    ...
    devices: 'All Devices',
    figmaUrl: 'https://www.figma.com/design/ghi789/example',
    feedbackStatus: null,
    noteIds: [],
    noteCount: 0,
  },
...
function ClientReviewMockupsInner() {
  ...
  const { data: rawExperiments, isLoading: experimentsLoading } = useAirtable<Record<string, unknown>>('experiments', {
    fields: [
      'Test Description', 'Hypothesis', 'Placement', 'Placement URL', 'Design Brief',
  ...
  const { data: rawNotes, mutate: mutateNotes } = useAirtable<Record<string, unknown>>('notes', {
    fields: ['Note', 'Experiments', 'Created Time', 'Experiment Type', 'Created By (Team)', 'Created By (Client)', 'Visibility'],
    filterExtra: noteFilter,
    sort: [{ field: 'Created Time', direction: 'desc' }],
    enabled: allNoteIds.length > 0 && !preview,
  })

  const notesByExperiment = useMemo<Map<string, { note: string; createdTime: string; type: string; createdByClient?: string; visibility?: string }[]>>(() => {
    const map = new Map<string, { note: string; createdTime: string; type: string; createdByClient?: string; visibility?: string }[]>()
    ...
    for (const rec of rawNotes) {
      ...
      for (const eid of expIds) {
        ...
        map.get(eid)!.push({ note, createdTime: time, type, createdByClient, visibility: vis })
      }
    }
    return map
  }, [rawNotes])
  ...
  const decisions = useMemo(() => {
    ...
    for (const exp of experiments) {
      if (exp.feedbackStatus === 'Client Approved Design') {
        merged.set(exp.id, 'approved')
      } else if (exp.feedbackStatus === 'Client Rejected Design') {
  ...
  const handleApprove = async (experimentId: string) => {
    ...
    try {
      const res = await fetch(`/api/airtable/experiments/${experimentId}`, {
        method: 'PATCH',
        headers: authHeaders,
        body: JSON.stringify({ fields: { 'Feedback Status': 'Client Approved Design' } }),
      })
      if (!res.ok) {
      ...
      try {
        ...
        await fetch(
          `https://more-conversions.app.n8n.cloud/webhook/585f1960-4886-4a68-b1d1-e6f1fa6fe0ec?experimentID=${experimentId}&batchID=${batchId}&clientID=${clientId}&action=client_approved_design`,
          { method: 'GET' },
        )
  ...
  const handleReject = async (experimentId: string) => {
    ...
    try {
      // 1. Create rejection Note with "Client Design Feedback" type
      const noteFields: Record<string, unknown> = {
        Note: feedbackText.trim(),
        Experiments: [experimentId],
        'Experiment Type': 'Client Design Feedback',
      }
      if (user) {
        if (user.role === 'client') {
      ...
      const patchRes = await fetch(`/api/airtable/experiments/${experimentId}`, {
        method: 'PATCH',
        headers: authHeaders,
        body: JSON.stringify({ fields: { 'Feedback Status': 'Client Rejected Design', 'Rejected At': new Date().toISOString() } }),
      })
      if (!patchRes.ok) {
      ...
      try {
        const clientId = batch?.clientId ?? user?.clientId ?? ''
        await fetch(
          `https://more-conversions.app.n8n.cloud/webhook/585f1960-4886-4a68-b1d1-e6f1fa6fe0ec?experimentID=${experimentId}&batchID=${batchId}&clientID=${clientId}&action=client_rejected_design`,
          { method: 'GET' },
        )
      } catch { /* webhook fire-and-forget */ }
  ...
  if (!preview && experiments.length === 0 && !batchLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px] text-[13px] text-neutral-500">
        No designs found for this batch.
      </div>
    )
  }
            ...
            <div className="flex items-center gap-3">
              ...
              {rejectedCount > 0 && (
                <span className="text-[12px] font-medium text-red-600">
                  {rejectedCount} {rejectedCount === 1 ? 'change' : 'changes'} requested
                </span>
              )}
              {allReviewed && (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1 ml-0">
                        ...
                        {exp.figmaUrl ? (
                          ...
                          <span className="text-[10px] text-amber-600 font-medium">No design link</span>
                        )}
                        ...
                        <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-[13px] text-amber-700 font-medium">
                              No design link has been submitted for this test yet.
                            </p>
                          </div>
                        </div>
                      )}

                      {/* Detail fields — client-friendly context */}
                      <div className="space-y-3">
                        <DetailField label="What this test does" value={exp.hypothesis} />
                        <DetailField label="Design Brief" value={exp.designBrief} />
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <DetailField label="Page" value={exp.placement} />
                          <DetailField label="Page URL" value={exp.placementUrl} isUrl />
                        </div>
                              ...
                              <div key={i} className="text-[13px] text-neutral-700 whitespace-pre-wrap bg-white rounded-lg border border-neutral-100 px-3 py-2">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  {n.createdByClient
                                    ...
                                    : <span className="text-[10px] text-neutral-500">Your Team</span>
                                  }
                                  {n.createdTime && (
                        ...
                        <div className="rounded-lg border border-red-200 bg-red-50/50 p-4 space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-[13px] font-medium text-red-800">
                              What changes would you like to this design?
                            </p>
                          </div>
                          <FormField label="Your feedback" required>
                            <textarea
                              ...
                              placeholder="Describe what you'd like changed in this design…"
                              rows={4}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[13px] text-emerald-600 mt-0.5">
                  You&apos;ve reviewed all {totalCount} {totalCount === 1 ? 'design' : 'designs'} in this batch.
                  {rejectedCount > 0
                    ? ` Your team has been notified and will make the requested changes.`
                    : ` Your team has been notified — these designs are ready to move forward.`
                  }
                </p>
              </div>
            </div>
        ...
        {confirmAction.type === 'approve' ? (
          ...
          <>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-neutral-800">Request changes to this design?</h3>
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col"
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1">
          <NotesPanel
            ...
            experimentType="Client Design Feedback"
...
export default function ClientReviewMockupsPage() {
  return (
    <FormPage>
      <ClientReviewMockupsInner />
    </FormPage>
  )
}
